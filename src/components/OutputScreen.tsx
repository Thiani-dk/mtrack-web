...
import { ArrowLeft, Inbox, Download, Tag, X, ChevronDown } from 'lucide-react';
        ...
        >
            <div className="p-1.5 space-y-0.5">
                ...
                {showCustom && (
                    <div className="px-2 pb-1 flex gap-1">
                        ...
                        <button
                            ...
                            className="text-xs bg-green-500 text-white px-2 rounded-lg"
                        >
                            ✓
                        </button>
                    </div>
                ...
                {current && (
                    <button
                        onClick={clear}
                        className="w-full text-left text-xs px-3 py-2 rounded-lg hover:bg-red-50 text-red-500"
                    >
                        Remove label
                    </button>
                )}
...
export function OutputScreen({ mode, transactions: initialTransactions, dateRangeLabel, onReset, onBack }: OutputScreenProps) {
    // Local copy so we can mutate receiptLabel / excludedFromReceipt without touching parent
        ...
        <motion.div className="flex flex-col min-h-screen bg-gray-50"
            ...
            <div className="flex-1 px-4 pt-5 pb-10 space-y-5 max-w-md mx-auto w-full">
                ...
                <motion.div className="bg-white rounded-2xl overflow-hidden"
                    ...
                    {/* Fees row — only shown if any fees were parsed */}
                    {totalFees > 0 && (
                ...
                {/* ── Receipt builder — only for receipt mode ── */}
                {mode === 'receipt' && (
